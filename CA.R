...
# Create initial n x n grid and populate it with random values to assign initial cell states. For each cell,
# if the value is equal or less than P_HIV, state = 4 (Infected), otherwise state = 1 (Healthy)
grid = matrix(runif(n*n), ncol = n);
grid = ifelse(grid <= P_HIV, 3, 1);

#Set the edges of the grid to Healthy state
grid[,c(1, n)] = 1
grid[c(1, n),] = 1
...
# initializes a grid to keep count of cells in every state
states_count = matrix(0, nrow = totalsteps, ncol = 3)
colnames(states_count) <- c("Healthy", "Infected", "Dead")

# Initialize a grid to track the number of timesteps that a cell has been in state I 
taugrid = matrix(0, nrow = n, ncol = n)
...
while(timestep <= totalsteps){
  	nextgrid = grid;
  		for (x in 2:(n-1)){
  	  		...
  	  		# is in I state then the cell becomes I with a probability of 
   	 		  ...
   	 		  if(grid[x,y]==1){
   	 		    c1 = runif(1)<=P_i;
   	 		    c2 = grid[x-1,y-1]==3 || grid[x-1,y]==3 || grid[x-1,y+1]==3 || grid[x,y-1]==3 || 
   	 		       + grid[x,y+1]==3 || grid[x+1,y-1]==3 || grid[x+1,y]==3 || grid[x+1,y+1]==3
  	    		c3 = runif(1)<=P_v
    		  ...
    		  # Rule 2
          # If the cell is in I state, and has been in this state for
          ...
          # P_repI (3.b)
          if(grid[x,y]==2 && runif(1)<=P_rep){
              nextgrid[x,y] = ifelse(runif(1)<=P_repI, 3, 1)
          }
  	...
  	states_count[timestep,1] = sum(grid[] == 1)
  	states_count[timestep,2] = sum(grid[] == 3)
  	states_count[timestep,3] = sum(grid[] == 2)
